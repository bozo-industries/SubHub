...
- Consent-first Money Rules: new detector tracks can add locally bounded EUR entries with daily/weekly caps and a correction window; settlement uses explicit PayPal approval and a separate backend, while clearing an unpaid balance never creates a charge
- Visual identity: three fixed primary tabs—Censor, Limits, and Money—use a centered bottom pill, violet active capsule, purpose-built icons, dense utility cards, and adaptive phone/tablet gutters. Secondary tools retain the same palette and card language without crowding the primary navigation
