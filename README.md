...
- Consent-first commitment: bounded 30-minute to 7-day keeper-code pacts seal app configuration with an explicit countdown and unconditional safety release; the pact itself never controls Device Admin, data clearing, or immediate protection stop
...
- Consensual Hardcore Mode: an optional Dom-only toggle launches Android's own Device Admin approval, declares no wipe/password/camera/lock/monitoring policies, and adds only the supported deactivate-before-uninstall step plus boot re-arming of App Mode intent
