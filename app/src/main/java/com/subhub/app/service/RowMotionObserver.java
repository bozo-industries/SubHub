...
final class RowMotionObserver {
    ...
    private long time, document;
    private int window, width, height, top, bottom;

    Sample observe(int[] pixels, int width, int height, int top, int bottom,
            long document, int window, long timestamp, boolean recentScrollSignal) {
        if (pixels == null || width < 4 || height < ROWS || width > 512 || height > 512
                || (long)width*height != pixels.length || top < 0 || bottom > height
                || bottom-top < ROWS || timestamp < 0 || document < 0 || window < 0) {
        ...
        boolean compatible=previous!=null && this.document==document && this.window==window
        ...
        previous=current; time=timestamp; this.document=document; this.window=window;
        this.width=width; this.height=height; this.top=top; this.bottom=bottom;
