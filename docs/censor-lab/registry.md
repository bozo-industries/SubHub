...
| [27](passes/pass-27.md) | planned | Bounded content-space memory and first-vsync re-entry | 26 | Cache only confirmed world regions; returning content is immediate, cache never becomes a delayed post-commit authority, and unseen visual pixels are not fabricated. |
...
- Current worktree: Pass27 planned from the retained Pass26 transaction/tick core. It must not overwrite the
