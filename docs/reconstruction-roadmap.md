...
10. Battery-aware Always-On/App Mode. **Complete with a styled launcher-app picker, exact package matching, all-app and selected-app modes, idle screenshot/inference suspension, IME transition handling, persisted last armed state, pact-aware non-exported boot restoration, a visible resume/disarm notification, and a fresh Android prompt for MediaProjection. Optional Hardcore Mode can restore armed intent but does not claim Device Admin grants capture or keepalive.**
...
13. Hardcore Mode. **Complete as an explicit Dom-only Device Admin opt-in with no declared device policies, Android-controlled activation/revocation, deactivate-before-uninstall friction, and boot re-arming of App Mode intent.**
