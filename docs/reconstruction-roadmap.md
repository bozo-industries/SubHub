...
7. Device/emulator parity testing and performance presets. **The API 35 automated suite now covers 40 instrumented contracts alongside unit, assembly, and lint checks; physical-device, long-run performance, and private positive-detection corpus coverage remain release QA.**
...
11. Daily selected-app budgets. **Complete with independently optional per-app and combined limits, foreground-only accounting, local-calendar reset, supported Android Home enforcement, compact App Mode controls, instrumentation coverage, and a live API 35 boundary test.**
