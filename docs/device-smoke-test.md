...
The source app builds, installs, launches, persists settings, obtains explicit Android screen-capture consent, processes captured frames with the local ONNX model, draws a touch-through overlay, and stops cleanly.
...
- Main and Settings screens launched through normal in-app navigation with no fatal exception.
...
- Every editable form field now retains a label and short explanation above its value, including Limits budgets, Wallet amounts, profile names, and custom phrases.
...
- The controller boundary covers Wallet, Limits, censor configuration, custom images, profiles, packs, Popup Storm, diagnostics, timer setup, permission repair, and destructive export settings. Locked pages remain readable; Enter Service stays available, while an active timer requires Dom mode to stop.
