...
| Visible explicit-text detection without OCR | New maintained App Mode feature | Working from bounded Accessibility node text with strict/balanced/broad sensitivity and category switches; fused with visual boxes before stable tracking so repeated screenshots and overlapping detections count once |
...
| Consensual Hardcore Mode | New maintained feature | Dom-only, opt-in Android Device Admin activation adds the platform's deactivate-before-uninstall step and boot re-arming of Accessibility App Mode intent. It requests no wipe, password, camera, force-lock, or monitoring policy; Android Settings revocation, Force stop, data clearing, fresh capture consent, and immediate protection stop remain available |
