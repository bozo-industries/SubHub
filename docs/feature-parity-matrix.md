...
| Controller read-only/edit boundary | New maintained feature | Working across every settings-changing primary and secondary page with a text-only EDIT/EDITING control, controller-PIN session, readable locked state, and explicit exemptions for safety exits and ordinary non-configuration operations; API 35 activity contracts cover the secondary surfaces |
